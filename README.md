...
# Discussion
La concurrence entraîne de gros ralentissement. Les requêtes timeline attaquent le serveur sans qu'il ne puisse s'y préparer, et une file d'attente va se mettre en place.  
Il est également possible, comme annoncé plus haut, que la commande xargs créée elle même cette file d'attente, ne sachant comment paralléliser 1000 commande ab en simultané.

On n'observe pas de ralentissements significatifs lorsque le nombre de posts et le nombre de follows augmente. Cela s'explique par le fait que la requête de timeline se limite à 20 posts. De plus le Datastore google cloud créé un index shardé, ce qui fait que même lorsque la taille des données augmente, on n'a pas à parcourir l'entièreté de la base de données mais simplement le shard où les données recherchés sont indexées.  
Cependant, il est possible que des variations soient cachées par la commande parallel.
